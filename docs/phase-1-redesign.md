...
2,053 tests green; the exit-signal gate passes with a verified instrument.
...
Since that line was written: **158 commits, 1,603 tests, 52 API endpoints, 88
...
document names ZERO of the 52 API endpoints that already exist.**
...
```
52 endpoints · 42 declare a response_model · 102 schema classes · 10 untyped
...
| **4.3** | 🟡 **HALF DONE, and the half that is not is a DECISION rather than an omission.** ✅ **`Why` is built and routed** — the content existed and was scattered (the track record inside `Decide`, the factor evidence inside `Research`, coverage inside `Screener`), and scattered it never answers the question somebody actually has, which is asked about the FRONT page. It opens with **what the app cannot do**, numbers included (*ranking on past three-year return put the worse quartile on top by 0.9pp, 19 of 44 windows*), because a scoreboard that leads with its wins is marketing. `SOURCES` is a **list, not a paragraph** — a row per figure, each naming AMFI, SEBI, the Income Tax Act or IIM Ahmedabad, and a test refuses the words *"our model"*, *"our algorithm"* and *"proprietary"* as sources. Each row also says what the figure is WORTH, and the page explains why `n/a` appears so often: a zero where we could not measure reads as good news. ⏸ **`Today` is NOT built** — `Today` and `Holdings` are one file and splitting `Portfolio.tsx` is **decision 2**, a product call about which of the two keeps `/portfolio`. BUILD.md says in as many words not to start before it lands. **The three honesty states are asserted to survive** regardless: a `misnamed_as` holding still says so, a stale price still says *"not current"*, a `price_error` holding is still excluded by name. | 2 |
...
the ENTIRE app so far    158 commits · 20 active days · 56 calendar days
                         52 endpoints · 97 schemas · 7 migrations
                         2,053 tests · a 5.2M-row NAV store · 63 .tsx files
