...
sense that cannot be tested from a document** — **one hundred forty-six review passes**
...
**48 things are open. 37 were open and are now closed.**
...
| `Manan` **Manan's real portfolio** | Not read. Chrome extension permission for `groww.in` still not granted. Design currently rests on a plausible portfolio, not his. 🟢 **Pass 134 narrowed what this actually blocks, and it is three questions, not the design.** The assumed portfolio is visible in the document — **five funds, ₹2,47,827, XIRR +9.4%** — so the worry was that decisions were tuned to it. Checked: **nothing in the advisor gates on portfolio size.** The badge vocabulary is relative or mechanical throughout — *₹5,000/yr cheaper elsewhere* prints a **computed** saving rather than firing on a rupee floor, overlap fires on a percentage, plan-type and *Too new to rank* are mechanical. The only absolute rupee constant in the advisor is `MIN_MONTHLY_SIP = 500`, which is a fund's own minimum, not an assumption about him; concentration is `0.30`, a share. **So the data layer, the scorer, the refusals and the badge grammar all hold whatever he owns.** What his portfolio genuinely settles: **(1)** whether look-through is worth building at all — it needs two or more equity funds that actually share holdings, and §3.3 calls it the largest missed move on the assumption that they do; **(2)** §3.2's unsettled density call, which reads differently at twelve rows than at three; **(3)** which badges ever fire — a portfolio holding no regular plans never shows the one §11.7 calls the largest number this app will ever display. **That is a much smaller ask than the row implied, and it is still the one thing on this list that changes what gets built.** |
