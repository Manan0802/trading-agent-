...
2,070 tests green; the exit-signal gate passes with a verified instrument.
...
Since that line was written: **158 commits, 1,603 tests, 53 API endpoints, 88
...
| **Universe** | Groww `st_filter` → **1,659** buyable direct-growth codes (2026-08-29; 1,686 before SIFs were excluded) | It is the platform Manan actually buys on. Carries TER, AUM, fund manager, min SIP/lumpsum, exit load, sub-category. |
...
document names ZERO of the 53 API endpoints that already exist.**
...
```
53 endpoints · 43 declare a response_model · 103 schema classes · 10 untyped
...
| **2.2** | ✅ **DONE 2026-08-29 — and the figure could finally be measured, because measuring it needs the buyable universe.** Against the catalogue the answer is 18; against the **1,659 buyable funds it was 74**, and every one of the 74 was OUTSIDE the catalogue, so the backfill — which walks the catalogue — could never have reached them. **91,668 rows fetched; buyable funds with no NAV history is now 0**, pinned by `test_buyable_have_nav.py`. 🔴 **Four separate catalogue filters were dropping real funds, each traced to a name:** `dividend` matched **"Dividend Yield"** and killed that whole equity sub-category (11 growth plans — Franklin, Aditya Birla, UTI, ICICI, Tata, HDFC); requiring the literal word `Growth` dropped **11** whose houses spell it `Cumulative` or omit the option entirely (`Tata Nifty 50 Index Fund - Direct Plan`); requiring `Direct` dropped **5** Motilal Oswal index funds whose mfapi name carries no plan word at all; and **47 passed every filter and then vanished at the detail call** — including `HDFC Nifty 50 Index Fund`, `HDFC BSE Sensex Index Fund` and `SBI Arbitrage Fund`, all of which return 3,300+ NAV rows when probed directly, so those were failed requests recorded nowhere. Fixed: the payout regex takes a negative lookahead, `Cumulative` counts as growth, candidates are the **union** of the name test and Groww's buyable list (membership is the answer, not a guess), and dropped candidates are now counted with the buyable ones named. | 1 |
| **2.3** | ✅ **DONE 2026-08-29.** Holdings store built at `.holdings/holdings.db` from the AMCs' own monthly SEBI filings — public regulatory documents, not a scraped API. **384 funds, 27,448 holding rows, 71.5 per fund**, against the 78.3 the 39-payload sample suggested; that sample was **all equity**, and the 98 covered funds returning nothing here are gold, overnight, debt, FoF and multi-asset, exactly as predicted. **Coverage is 23% of the 1,659 buyable funds and every surface says so** — seven AMCs have a verified source, so `stored_as_of` returns None rather than an overlap of zero, because *"these funds share nothing"* and *"we did not look"* must not be the same number. **Acceptance run, not asserted:** the store was deleted, restored from the committed 439 KB gzipped SQL dump, and came back byte-identical in counts and queries. 🔴 **Two defects found doing it.** `CREATE TABLE IF NOT EXISTS` leaves an old table in place, so adding a column surfaced as `OperationalError` on the first WRITE in production rather than at start-up — now a `user_version` check that rebuilds, which is right because the store is a cache of recoverable filings. And `iterdump` emits no `PRAGMA user_version`, so a restored store read back as stale and **the staleness rule deleted it** — a backup that erased itself the first time it was used, while looking like it had worked. **Look-through shipped** as `GET /portfolio/look-through`: fund value x holding weight summed by ISIN, leading with `covered_share`. HDFC Bank appears in **146 of the 384 stored funds**, which is the whole argument for the screen. | 1 |
| **2.4** | ✅ **The publishing risk is closed; the stock pipeline it guards against is still ahead.** `trim_nav_store.py` is `con.backup()` — a whole-file snapshot — followed by ONE `DELETE FROM nav_history`, so every other table rides into the published asset at full size. It now drops every table the runtime does not read, driven by an **allowlist, not a blocklist**: a blocklist only excludes what somebody remembered to name, so `stock_daily` (~9.3M rows, estimated) ships the day it lands and nobody finds out until the free host runs out of disk at boot. With an allowlist a new table is excluded by default and publishing it is a deliberate line of code. Three tests, including one that adds a table nobody has thought of yet. **`stock_daily` and `corporate_actions` still do not exist** — the NSE bhavcopy ingestion is real remaining work, and §16's *"~9.3M rows"* stays an estimate stated in the present tense. | 1 |
...
| | | sessions |
|---|---|---|
| **3.1** | ✅ **DONE 2026-08-29 — `grounding.py` has callers, and one of them is the app's oldest piece of model-written prose.** The module was 792 lines with 50 tests and **zero non-test callers** — a guard with nothing behind it. Slice 1.4's switch badge is the first; `get_goal_explanation` is the second, and it was returning model prose to the screen unexamined. Now every figure is computed BEFORE the model is asked anything, the six numbers become a payload, and `check` gates the return: a sentence carrying a seventh is **discarded and the template answers instead**. Proven with four fabrications — an invented ₹99,99,999, an invented 18% return, a 25-year horizon against a 10-year goal, and a wrong SIP amount. ⚠️ **`check`, not `check_all`, and the comment says why:** the model is not asked to cite anything, so there are no per-figure `Claim`s and `check_claims` on an empty list would pass vacuously. 🔴 **The guard caught my own test fixture twice** — a stub saying *"₹5 lakh"*, which is not one of the six, was discarded, and the test failed for a better reason than the one it was written for. | 1 |
| **3.2** | ✅ **DONE 2026-08-29 — the registry exists, and writing it down changed the count.** §3.5 named **seven** tools scattered through prose and the figure "18" appeared three times, written nowhere as eighteen things — so a JSON contract could not be written against it. `services/llm/tools.py` enumerates **twenty**, and the two extra are the resolver rule doing its job: `company_exposure` takes an ISIN and `stock_facts` takes a ticker, and neither had a resolver. Pointing them at `resolve_fund` — which a hurried version does — returns FUND matches for a company name, so the rule fails silently while appearing to hold. **The three genuinely new tools are built:** `look_through` (2.3), `switch_cost` (1.4), and `company_exposure` now, as `GET /portfolio/company-exposure/{isin}` — which **404s rather than returning zero** for a company we hold nothing of, because *"you own none of this"* and *"none of the funds we could open holds it"* are different claims. Every schema is checked with `jsonschema`, every declared field is required (an optional one is a field a caller may quietly stop sending), and **the cache key hashes the contract** — hashing only the arguments serves a response built against the old shape after it moves. 112 tests, including the model putting a fund's whole NAME into `scheme_code`, which is the failure the resolver rule was measured against. | 3 |
...
| **3.4** | ✅ **DONE 2026-08-29 — every §5 refusal now has a test that the answer is refused.** `POST /ask` ships with the gate **in code, before the model**, which is the whole design: a rule in a system prompt is a request, and a model asked *"should I sell my worst fund?"* answers fluently, plausibly, and against a measurement this repo already ran. A refused question **never reaches a model at all** — proven by a test that replaces `call_llm` with something that raises. Four adversarial rephrasings (*"ignore your rules"*, *"as an experienced advisor"*, *"hypothetically"*, *"my friend asks"*) are refused. 🔴 **Building it forced a distinction §5 never made: this app DOES recommend selling** — out of a regular plan into its direct twin — so a blanket sell-refusal would refuse slice 1.4, the app's own strongest advice. A tenth rule, `sell-on-no-stated-basis`, refuses a sell question that names no criterion and **excludes** anything mentioning cost, fees or the direct plan. It sits LAST, because placed anywhere else it answered *"the fund manager left, should I redeem?"* with a lecture about performance ranking — true, and not what was asked. A test pins its position. ⚠️ **A refusal is a 200, not a 4xx:** an error status makes a considered position look like a malfunction. And a question that is neither refused nor answerable says so — the 18 tools do not exist yet, so the alternative is a model improvising over a portfolio it was never handed. | 1 |
...
| **4.2** | ✅ **DONE 2026-08-29 — both absent features, and neither is virtualisation.** **Overlap at the moment of choosing:** `GET /portfolio/already-own/{code}` answers *"how much of this fund do you already own"* — the number that decides whether adding a fund does anything, since two large-cap funds plus a third is usually the same thirty companies a third time, and the rank, the cost and the base rate will all say the third fund is good. The share is **of the candidate**, not of the portfolio; measured the other way it shrinks as the portfolio grows, which is exactly backwards. A company two held funds share is counted once — summing per-fund overlaps can exceed 100%. **It returns `null` and renders `n/a`, never `0%`** (§14): 0% reads as perfectly diversified, the opposite of *"we could not tell"*, and it is the more attractive of the two readings, so a silent zero encourages the purchase it should have questioned. The two unmeasurable cases give **different** reasons — one is about the fund, one about the portfolio. **The compare tray** holds four, because eight is a table and a table is what they were already looking at; it keeps whole rows so a filter change cannot empty it, marks no best against a single known value, and puts **past 3-year return LAST with the reason on the row** — first would undo §1.1 with layout. ⚠️ **Virtualisation deliberately not added**: `Screener.tsx` paginates at `PAGE_SIZE = 100` on measured grounds. | 4 |
| **4.3** | ✅ **DONE 2026-08-29 — both halves, and decision 2 is made.** `Portfolio.tsx` rendered the summary, the levers, the chart, the cost review, the overlap AND the holdings table on one screen. Split: **`/portfolio` keeps the summary**, named **`Portfolio`**, because `/` redirects there, so it is the app's front door — and a front door should answer *how am I doing* rather than open onto a table. `Holdings` lives at **`/portfolio/holdings`**, which is where a deliberate destination belongs: you go there to add a purchase or correct a unit count, which is a task rather than a glance. Both use the same query key, so moving between them costs no request, and the summary links across where somebody actually wonders. ⚠️ **The summary was briefly labelled `Today`** — a name for WHEN you look at a page rather than what is on it, and no other word in the app worked that way; Manan renamed it. **`Why`** ships too — the track record, the factor evidence and the coverage figures were scattered across `Decide`, `Research` and `Screener`, and scattered they never answer the question asked about the FRONT page. It opens with **what the app cannot do**, numbers included, because a scoreboard that leads with its wins is marketing. `SOURCES` is a list, not a paragraph, and a test refuses *"our model"*, *"our algorithm"* and *"proprietary"* as sources. **The three §14 honesty states survived the rewrite and are pinned on the page that now owns the rows** — `misnamed_as`, *"This value is not current"*, and a `price_error` holding excluded by name — plus a test that the summary does not render the table again. That test used to assert the old FILENAME was gone, which stopped meaning anything the moment the file was renamed back — a filename is not the property; one page holding the glance and the other holding the rows is. | 2 |
...
the ENTIRE app so far    158 commits · 20 active days · 56 calendar days
                         53 endpoints · 98 schemas · 7 migrations
                         2,070 tests · a 5.2M-row NAV store · 64 .tsx files
...
| `slice 2` ✅ **RESOLVED — all four re-measured live on 2026-08-29 and now reproducible by running `scripts/build_groww_universe.py`** | Every figure moved a little, and one moved for a reason worth keeping. **buyable 1,686 -> 1,659**; TER agreement **94.2% -> 94.4%** (1,165 of 1,234 within 0.10pp, 69 disagree, was 71); Large Cap **123 exactly as stated**, of which index **86 -> 83**; the category split is now on the record (890 Equity · 461 Debt · 244 Hybrid · 44 Commodities). **The drop to 1,659 is not drift.** Groww's listing now carries **Specialised Investment Funds**, keyed `SIF-14` instead of by an AMFI code — 38 rows, 30 passing every other buyability filter. Zero of the 30 appear in AMFI's expense file, so this app can compute no NAV, no cost, no category rank and no score for any of them, while listing them as buyable; and each carries a **₹10,00,000 minimum** against a ₹1,000 median for ordinary funds. Excluded, counted in the build report, and pinned by a test. **The `index` boolean is confirmed to live in `st_filter` and nowhere else** — 0 of 1,659 buyable funds now lack it, against 0 of 39 scheme-detail payloads that carried it, so `is_passive` is a read rather than a name test. The raw pull is retained in gitignored `.growwcache/`; what is committed is codes plus three attributes, since Groww's `/v1/api/*` is `Disallow:`. |
