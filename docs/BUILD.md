...
| **2.1b** | ✅ **Measured, and it changed the claim rather than confirming it.** Ranking each decision date on the TER filed THAT date, within category: the effect survives but the magnitude falls from **+12.0pp to +3.6pp** and the win rate from 100% to 50%. §1.1 now states both. ⚠️ **Sample: 6 and 4 category-windows** — far too thin to put a number on §1.1's own +2.1pp, and the two are not comparable (different houses, different coverage). What it establishes is the SHAPE: the honest measurement is smaller than the published one and still positive, and **the lookahead is not harmless** — the script assumed ranking on today's filing changes nothing, and here it changes the sign. | 1 |
...
2. ✅ **DECIDED and built 2026-08-29.** `/portfolio` keeps the **summary** — `/` redirects there, so it is the front door, and a front door should answer *how am I doing* rather than open onto a table. **`Holdings` is at `/portfolio/holdings`**: you go there to add a purchase or fix a unit count, which is a task, not a glance. Same query key, so the move costs no request.
